...
rightTwo = \relative c'' {
  ...
  % Music follows here.
  s1*2 %\bar 2
\omit TupletNumber\omit TupletBracket  <a, c>4\accent  c'8.-\tweak #'Y-offset #0 -\tweak #'X-offset #-3 -2\accent   \change Staff = "left" \stemUp c16^\accent c4^2^1^\accent c,8.^\accent \change Staff = "right" \stemDown c16_2\accent %\bar3
  <a c>4\accent \tuplet 3/2{ d'8\accent^\( \change Staff = "left" \stemUp c8.^\accent b16^\accent\)} b4^1^\accent \tuplet 3/2 {d,8^2^1^\accent\( \change Staff = "right" \stemDown c8.\accent b16\accent}%bar 4
 \tuplet 3/2{ b4\accent\) <ais' ais'>8\(}\tuplet 3/2{ < b b' -\tweak #'Y-offset #-2 -\tweak #'X-offset #-3 -5>8 _\< < dis dis'>8 <gis gis'>8\! }\tuplet 3/2{<ais e'>4\) s8} \tuplet 3/2{\once \override NoteColumn.force-hshift = #0.1   e,4 <fis, ais>8} \tuplet 3/2{ ais[( b)]  ais'[( b\accent)] <gis' gis'>8 <fis fis'>8} b4\accent s4
}
...
\tuplet 3/2{\once\override Beam #'positions = #'(6 . 9) \once\override Hairpin #'rotation = #'(5 -1 0) b16_\<-\tweak #'X-offset #0 -\tweak #'Y-offset #2 -5 -\tweak #'X-offset #0 -\tweak #'Y-offset #3 -1[ fis-\tweak #'Y-offset #0 ^4 a-\tweak #'Y-offset #0 ^3 c-\tweak #'Y-offset #2 ^2 b' -\tweak #'Y-offset #4 ^1 \once \override Stem.beaming = #(cons (list 1 0) (list 1)) fis-\tweak #'Y-offset #3  -4 \once \override Stem.beaming = #(cons (list 0) (list -1 0)) \once \override Stem.beaming = #(cons (list 0) (list -1 0)) a-\tweak #'Y-offset #4 ^3 c-\tweak #'Y-offset #0 -2 b'-\tweak #'Y-offset #0 _1 fis -\tweak #'Y-offset #0 _3 a-\tweak #'Y-offset #0 _2 c,]\!}\stemDown  \once\override Beam #'positions = #'(-4 . -6)   \tuplet 3/2{a'16_\>[\sustainOn e^4 g^3^2_1 c,^5_2 a^1_3 \once \override Stem.beaming = #(cons (list 1 0) (list 1)) e^3_5 \once \override Stem.beaming = #(cons (list 0) (list -1 0))  \once \override Staff.SustainPedal.stencil =
  	#(lambda (grob) (grob-interpret-markup grob 
...
\tuplet 3/2{\stemUp e16_\<[ b_4-\tweak #'Y-offset #2 ^3 dis_3-\tweak #'Y-offset #2 ^2 fis-\tweak #'Y-offset #-1 _2 -\tweak #'Y-offset #2 ^1 e'-\tweak #'Y-offset #2 _1-\tweak #'Y-offset #5 ^2 \once \override Stem.beaming = #(cons (list 1 0) (list 1)) b-\tweak #'Y-offset #1 _5  \once \override Stem.beaming = #(cons (list 0) (list -1 0))\once \override Stem.beaming = #(cons (list 0) (list -1 0)) \once \override Stem.beaming = #(cons (list 0) (list -1 0)) \once \override Stem.beaming = #(cons (list 0) (list -1 0)) dis-\tweak #'Y-offset #1 _3  fis-\tweak #'Y-offset #1 _2 \clef "treble" cis'_1 fis,_5 b_3 bis_2]}\tuplet 3/2{\stemUp  dis16_1_\>_\tweak #'Y-offset #-6 \sustainOn[ ais_3 cis_2 fis,_5 dis_1 \once \override Stem.beaming = #(cons (list 1 0) (list 1))  ais \once \override Stem.beaming = #(cons (list 0) (list -1 0)) cis fis, \clef "bass" dis_1 ais_2 cis_1 <b,! fis'>\!\sustainOn]}%bar5
s1
